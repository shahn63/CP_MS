...
sd.idx<-NULL
for(i in 1:length(ff.idx)){
	if(length(which(sd[,,i]==0))!=0){
		sd.idx<-c(sd.idx,i)
	}	
}
sd.idx<-ff.idx
for(i in 1:length(sd.idx)){
	sd[,,sd.idx[i]]<-(sd[,,sd.idx[i]]+t(sd[,,sd.idx[i]]))/2+diag(ncol(tr.dat))*0.001
}
...
for(i in 1:dim(cal.dat)[2]){
	...
	for(j in 1:length(ff.idx)){
		...
		mag.mh<-cbind(mag.mh, mh.ij-2*log(phi[j]))
	}
...
for(i in 1:dim(cal.dat)[2]){
	...
	uni.lambda<-uni.conf[ord.mh,i][idx.a]
}

Lt<-Ut<-matrix(0,nrow=length(ff.idx),ncol=ncol(cal.dat))
for(j in 1:length(ff.idx)){
	for(i in 1:ncol(cal.dat)){
		Ut[j,i]<-mu[i,j]+sqrt((2*log(phi[j])+uni.lambda)*sd[i,i,j])
		Lt[j,i]<-mu[i,j]-sqrt((2*log(phi[j])+uni.lambda)*sd[i,i,j])
	}
}
